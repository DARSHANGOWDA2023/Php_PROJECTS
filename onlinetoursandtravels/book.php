...
echo $uid;
    ...
    if(array_key_exists("cust",$_POST) OR array_key_exists("tcost",$_POST)){
                ...
                echo $_POST['pack_id'];
                $query= "INSERT INTO `booking` (`u_id`,`pid`,`no_pass`,`total`,`card_no`) VALUES ('$uid','".mysqli_real_escape_string($link, $_POST['pack_id'])."','".mysqli_real_escape_string($link, $_POST['cust'])."' ,'".mysqli_real_escape_string($link, $_POST['tcost'])."','".mysqli_real_escape_string($link, $_POST['card'])."')";
...
<body>
    ...
    <div id=class>
    <form method="post" action="book.php?bookid=<?=$bookid?>&amnt=<?=$amnt?>">
    <p>PACKAGE ID:<br>
    ...
    <p><input type="number" name="card"  placeholder="Enter card number" ></p>
        <p><input type="text" name="exp"  placeholder="mm/yy" ></p>
           <p> <input type="number(3)" name="cvv" placeholder="CVV" >  
    </p>   
        </div>
